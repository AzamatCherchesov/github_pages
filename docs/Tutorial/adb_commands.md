...
<br/> Для выполнения ADB команд устанавливается клиент-серверное взаимодействие между мобильным устройством (или эмулятором) и дектопом. Сперва нужно скачать файл `adbserver-desktop.jar` из официального гитхаба Kaspresso ![adbserver-desktop.jar](https://github.com/KasperskyLab/Kaspresso/blob/master/artifacts/adbserver-desktop.jar) и выполнить следующую команду в терминале:
````Java
java -jar <path/to/file>/adbserver-desktop.jar
````
<br/> Запустится AdbServer - реализация серверной части взаимодействия. При запуске теста девайс будет инициировать подключение к серверу для выполнения ADB команд на устройстве.

## Пример теста с использованием adb

<br/> Ниже приведен пример теста, который:
<br/> 1. Устанавливает заранее собранное приложение HelloWorld на девайс (HelloWorld был взят из официального гитхаба Kaspresso ![hello_world.apk](https://github.com/KasperskyLab/Kaspresso/blob/master/artifacts/hello_world.apk)
<br/> 2. Проверяет, что приложение успешно установилось
<br/> 3. Удаляет приложение HelloWorld с девайса
<br/> 4. Проверяет, что приложения HelloWorld нет в списке установленных.
...
class AdbTest : TestCase() {
    ...
    fun appsTest() = run {
        step("Install hello world apk") {
            adbServer.performAdb("install $TEST_APK_FILE_RELATIVE_PATH")
            Assert.assertTrue(isAppInstalled(adbServer, TEST_APK_PACKAGE_NAME))
        }

        step("Delete the application") {
            adbServer.performAdb("uninstall $TEST_APK_PACKAGE_NAME")
...
````
<br/> В Kaspresso для работы с adb есть специальная абстракция `AdbServer`. Экземлпяр этого класса доступен в `BaseTestContext` и в `BaseTestCase`. 
